...
nhanes_dis = clarite.load.from_tsv("Discovery_Results_python.txt")
...
for current_pheno in pheno:
    ...
    ewas_result_dis = clarite.analyze.ewas(outcome= current_pheno,
                                       ...
                                       min_n=200,
                                       data=discovery_all_white.drop(columns=drop_phenos),
                                       survey_design_spec=design_wh,
...
result.to_csv("Discovery_Results_python_stdbeta_wh.txt", na_rep="NA", sep="\t")
...
for phenotype, variable_list in sr_dict.items():
    print(phenotype)
    data = replication_all_white[[phenotype] + covariates + variable_list]
    ewas_result_rep = clarite.analyze.ewas(outcome= phenotype,
                                                 covariates= covariates, 
                                                 min_n=200,
                                                 data=data,
                                                 survey_design_spec=design2_wh,
                                                 standardize_data=True)
...
result_rep.to_csv("Replication_Results_python_stdbeta_wh.txt", na_rep="NA", sep="\t")

black_results = pd.read_csv("Discovery_Results_python_stdbeta_aa.txt", sep="\t")
black_results= black_results.set_index(['Variable', 'Outcome'])
black_results_rep = pd.read_csv("Replication_Results_python_stdbeta_aa.txt", sep="\t")
black_results_rep= black_results_rep.set_index(['Variable', 'Outcome'])
mexican_results = pd.read_csv("Discovery_Results_python_stdbeta_mx.txt", sep="\t")
mexican_results= mexican_results.set_index(['Variable', 'Outcome'])
mexican_results_rep = pd.read_csv("Replication_Results_python_stdbeta_mx.txt", sep="\t")
mexican_results_rep= mexican_results_rep.set_index(['Variable', 'Outcome'])
original_results = pd.read_csv("Discovery_Results_python_stdbeta.txt", sep="\t")
original_results= original_results.set_index(['Variable', 'Outcome'])
original_results_rep = pd.read_csv("Replication_Results_python_stdbeta.txt", sep="\t")
original_results_rep= original_results_rep.set_index(['Variable', 'Outcome'])
...
fdr_significant.to_csv("Significant_Results_FDR_0.1_std_white.txt", sep="\t")
...
bonf_significant05 = bonf_significant05.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant05.to_csv("Significant_Results_Bonferroni_0.05_std_white.txt", sep="\t")
...
bonf_significant01 = bonf_significant01.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant01.to_csv("Significant_Results_Bonferroni_0.01_std_white.txt", sep="\t")
...
# Save combined reports
    # Combine results
ewas_keep_cols = ['pvalue', 'pvalue_bonferroni', 'pvalue_fdr', 'N', 'Beta']
combined = pd.merge(black_results[['Variable_type'] + ewas_keep_cols],
                        black_results_rep[ewas_keep_cols],
                        left_index=True, right_index=True, suffixes=("_disc", "_repl"))
# FDR < 0.1 in both
fdr_significant = combined.loc[(combined['pvalue_fdr_disc'] <= 0.1) & (combined['pvalue_fdr_repl'] <= 0.1),]
fdr_significant = fdr_significant.assign(m=fdr_significant[['pvalue_fdr_disc', 'pvalue_fdr_repl']].mean(axis=1))\
                                     .sort_values('m').drop('m', axis=1)
fdr_significant.to_csv("Significant_Results_FDR_0.1_std_black.txt", sep="\t")
print(f"{len(fdr_significant)} variables had FDR < 0.1 in both discovery and replication")
...
bonf_significant05 = bonf_significant05.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant05.to_csv("Significant_Results_Bonferroni_0.05_std_black.txt", sep="\t")
print(f"{len(bonf_significant05)} variables had Bonferroni < 0.05 in both discovery and replication")
...
bonf_significant01 = bonf_significant01.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant01.to_csv("Significant_Results_Bonferroni_0.01_std_black.txt", sep="\t")
print(f"{len(bonf_significant01)} variables had Bonferroni < 0.01 in both discovery and replication")
...
# Save combined reports
    # Combine results
ewas_keep_cols = ['pvalue', 'pvalue_bonferroni', 'pvalue_fdr', 'N', 'Beta']
combined = pd.merge(mexican_results[['Variable_type'] + ewas_keep_cols],
                        mexican_results_rep[ewas_keep_cols],
                        left_index=True, right_index=True, suffixes=("_disc", "_repl"))
# FDR < 0.1 in both
fdr_significant = combined.loc[(combined['pvalue_fdr_disc'] <= 0.1) & (combined['pvalue_fdr_repl'] <= 0.1),]
fdr_significant = fdr_significant.assign(m=fdr_significant[['pvalue_fdr_disc', 'pvalue_fdr_repl']].mean(axis=1))\
                                     .sort_values('m').drop('m', axis=1)
fdr_significant.to_csv("Significant_Results_FDR_0.1_std_mexican.txt", sep="\t")
print(f"{len(fdr_significant)} variables had FDR < 0.1 in both discovery and replication")
...
bonf_significant05 = bonf_significant05.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant05.to_csv("Significant_Results_Bonferroni_0.05_std_mexican.txt", sep="\t")
print(f"{len(bonf_significant05)} variables had Bonferroni < 0.05 in both discovery and replication")
...
bonf_significant01 = bonf_significant01.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant01.to_csv("Significant_Results_Bonferroni_0.01_std_mexican.txt", sep="\t")
print(f"{len(bonf_significant01)} variables had Bonferroni < 0.01 in both discovery and replication")

#Save combined reports
    # Combine results
ewas_keep_cols = ['pvalue', 'pvalue_bonferroni', 'pvalue_fdr', 'N', 'Beta']
combined = pd.merge(original_results[['Variable_type'] + ewas_keep_cols],
                        original_results_rep[ewas_keep_cols],
                        left_index=True, right_index=True, suffixes=("_disc", "_repl"))
...
fdr_significant = fdr_significant.assign(m=fdr_significant[['pvalue_fdr_disc', 'pvalue_fdr_repl']].mean(axis=1))\
                                     .sort_values('m').drop('m', axis=1)
fdr_significant.to_csv("Significant_Results_FDR_0.1_std.txt", sep="\t")
print(f"{len(fdr_significant)} variables had FDR < 0.1 in both discovery and replication")
...
bonf_significant05 = bonf_significant05.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant05.to_csv("Significant_Results_Bonferroni_0.05_std.txt", sep="\t")
print(f"{len(bonf_significant05)} variables had Bonferroni < 0.05 in both discovery and replication")
...
bonf_significant01 = bonf_significant01.assign(m=fdr_significant[['pvalue_bonferroni_disc', 'pvalue_bonferroni_repl']].mean(axis=1))\
                                           .sort_values('m').drop('m', axis=1)
bonf_significant01.to_csv("Significant_Results_Bonferroni_0.01_std.txt", sep="\t")
print(f"{len(bonf_significant01)} variables had Bonferroni < 0.01 in both discovery and replication")
