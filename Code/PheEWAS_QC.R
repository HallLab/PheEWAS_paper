...
data_main_table <- paste("/storage/home/n/nep63/group_mah546/datasets/NHANES/NHANES_99_06_patel_github/nh_99-06/MainTable.csv", sep="/")
data_var_description <- paste("/storage/home/n/nep63/group_mah546/datasets/NHANES/NHANES_99_06_patel_github/nh_99-06/VarDescription.csv", sep="/")
data_var_phenotypes_dis <- paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/var_text_phe.txt", sep="/", header = T)
read_over_18 <- paste("/storage/home/n/nep63/group_mah546/projects/NHANES_claritee/data/Adult/MainTable_keepvar_over18.tsv")
names(data_var_phenotypes_dis)
...
data <- read.csv(data_var_phenotypes_dis)

# Add survey year and combine LBDHDD and LBDHDL columns
main_table <- read.csv(data_main_table) 
colnames(main_table)[1] <- "ID" 
survey_year <- colfilter(main_table, c("ID")) 
main_table[main_table$SDDSRVYR==3 | main_table$SDDSRVYR==4, "LBDHDL"] = main_table[main_table$SDDSRVYR==3 | main_table$SDDSRVYR==4, "LBDHDD"] 
nhanes <- merge_data(survey_year, main_table, union = FALSE)

#make groups for race-ethnicty
n_white <- nhanes[nhanes$white==1, ]
...
#Get survey info
survey_list_dis_table <- colfilter(nhanes, survey_list_dis, exclude=FALSE)
survey_design_discovery <- read.csv(paste("/storage/home/n/nep63/group_mah546/personal/nikki/weights_discovery.txt", sep="/"), sep="\t")
colnames(survey_design_discovery)[1] <- "ID"
survey_design_discovery <- colfilter(survey_design_discovery, c("SDDSRVYR"), exclude=TRUE)

#Get survey info
survey_design_replication <- read.csv(paste("/storage/home/n/nep63/group_mah546/personal/nikki/weights_replication.txt", sep="/"), sep="\t")
...
var_weights <- read.csv(paste("/storage/home/n/nep63/group_mah546/personal/nikki/BMI_Survey_Weights/VarWeights_new.csv", sep="/"), fileEncoding="UTF-8-BOM", stringsAsFactors = FALSE)
...
# Remove physical fitness vars and unknown pesticide variable and inverted LBXSCRINV
no_pf_vars_URXP08 <- c("CVDVOMAX","CVDESVO2","CVDS1HR","CVDS1SY","CVDS1DI","CVDS2HR","CVDS2SY", "CVDS2DI","CVDR1HR","CVDR1SY","CVDR1DI","CVDR2HR","CVDR2SY","CVDR2DI","physical_activity", "URXP08", "LBXSCRINV") 
...
# Split MainTable variables by type
###########
...
#Adjust for statins (LDL=LDL/0.7)
...
#Extract LDL from continuous and merge with statin
...
keepvar <- read.delim("/storage/home/n/nep63/group_mah546/projects/NHANES_claritee/data/VarDescription_keepcategories_varnames.txt", header=FALSE)
keepvar <- keepvar[keepvar$V1!="male", , drop=FALSE]
...
phe_discovery_complete <- remove_incomplete_obs(phe_discovery_series, c(covariates))
phe_replication_filter <- phe_replication_series[!is.na('phenotype_rep'), ]
phe_replication_complete <- remove_incomplete_obs(phe_replication_series, c(covariates))

#Separate phenotypes and expsosure data to better follow the QC process with each type of variable
...
#Reflect any negative skewness values that are less that -0.5  
...
log_ID_refelct_dis <- log(ID_reflect_dis)
log_ID_refelct_rep <- log(ID_reflect_rep)

###Re-reflect the values and save images###
reflect_phe_max_log <- max(log_ID_refelct_dis, na.rm =TRUE)+1
reflect_phe_max_data_log <- reflect_phe_max_log-log_ID_refelct_dis[2:3]
reflect_phe_rep_max_log <- max(log_ID_refelct_rep, na.rm =TRUE)+1
reflect_phe_max_rep_data_log <- reflect_phe_rep_max_log-log_ID_refelct_rep[2:5]
ID_dis <- colfilter(discovery_cont_zero_phe, ID_name, exclude = F)
ID_reflect_dis_log <-cbind(ID_dis, reflect_phe_max_data_log)
ID_reflect_rep_log <-cbind(ID_rep, reflect_phe_max_rep_data_log)

neg_dis_log_pic <- hist_plot(ID_reflect_dis_log, n = 2, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/neg_dis_trans", nrow = 1, ncol = 3, wi = 10, hgt = 3, res = 300)
...
phe_dis_not_transformed <- hist_plot(discovery_cont_zero_phe, n = 12, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/phe_dis_not_transformed", nrow = 4, ncol = 3, wi = 13.5, hgt = 12, res = 300)
phe_rep_not_transformed <- hist_plot(replication_cont_zero_phe, n = 12, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/phe_rep_not_transformed", nrow = 4, ncol = 3, wi = 13.5, hgt = 12, res = 300)

#Remove variables that are not being transformed
dis_no_transform <- c("LBXLYPCT",	"LBXNEPCT",	"LBXHGB",	"LBXHCT",	"LBXSAL", "LBXSCA",	"LBXSPH",	"LBXSTP",	"LBXSUA",	"LBXSCLSI",	"LBXSOSSI", "LBXSC3SI", "LBXSNASI", "LBXMCVSI", "LBXMCHSI")
res_no_transform <- c("LBXLYPCT",	"LBXNEPCT",	"LBXRBCSI",	"LBXHGB",	"LBXHCT",	"LBXSCA",	"LBXSPH",	"LBXSUA",	"LBXSOSSI",	"LBXSC3SI", "LBXMCVSI", "LBXMCHSI", "LBXSAL", "LBXSCLSI")
...
log_one_dis_vars <- c("LBXEOPCT",	"LBXEPP",	"LBDEONO",	"LBDBANO", "LBXBAPCT")
...
#SKIP FOR NOW
dis_id_log <-cbind(ID_dis, dis_log)
one_dis_id_log <-cbind(ID_dis, one_log_dis)
dis_id_log <- dis_id_log[,-2]
pos_dis_log <- merge_data(dis_id_log, one_dis_id_log, union=TRUE)
pos_dis_log_pic <- hist_plot(pos_dis_log, n = 12, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/pos_dis_trans", nrow = 4, ncol = 3, wi = 10, hgt = 12, res = 300)

#Put all transformed and non-transformed columns back into one data frame
...
phe_dis_all <- hist_plot(all_dis_data, n = 12, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/phe_dis_all", nrow = 4, ncol = 3, wi = 13.5, hgt = 12, res = 300)
phe_rep_all <- hist_plot(all_rep_data, n = 12, file = "/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/phe_rep_all", nrow = 4, ncol = 3, wi = 13.5, hgt = 12, res = 300)
...
write.table(discovery_catbin_final_exp,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/discovery_catbin_final_exp.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
replication_catbin_final_exp <- merge_data(replication_cat_final_exp, replication_bin_final_exp, union = TRUE)
write.table(replication_catbin_final_exp,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/replication_catbin_final_exp.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
...
write.table(discovery_cont_phenotype_exp,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/discovery_cont_phenotype_exp.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
replication_cont_phenotype_exp<- colfilter(replication_cont_phenotype_exp, remove_variables, exclude=TRUE)
write.table(replication_cont_phenotype_exp,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/replication_cont_phenotype_exp.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
remove_variables2 <- c("current_loud_noise", "no_salt", "ordinary_salt", "white")
discovery_catbin_final_exp <- colfilter(discovery_catbin_final_exp, remove_variables2, exclude=TRUE)
replication_catbin_final_exp<- colfilter(replication_catbin_final_exp, remove_variables2, exclude=TRUE)

# Get typed variables and then merge data #
all_dis_data_list <- colnames(all_dis_data [2:57])
write.table(all_dis_data,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/all_dis_data.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
...
# Discovery Phe-EWAS - Weighted #Repeat for x times with input of 
#############################
...
write.table(discovery_results_phe_ewas,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/dis_all_results.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
write.table(significant_discovery_vars_all,
            file = paste("/storage/home/n/nep63/group_mah546/personal/nikki/PheWAS_NHANES/New_PheEWAS/sig_all_results.txt", sep="/"),
            sep = "\t", quote = FALSE, row.names = FALSE)
